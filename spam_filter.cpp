...
int main(int argc, char *argv[]) {
  ...
  // train_file_spam(".\\csv\\train\\dataset_spam_train100.csv");
  ...
  if (train_file_ham.is_open()) {
    ...
    while (getline(train_file_ham, str_buf, ',')) {
      ...
      if (first) {
        ...
        if (i == 101) {
          if ((line.size() > 1) &&
              (line.at(line.size() - 1)[line.at(line.size() - 1).size() - 1] ==
               '\"')) { // 진짜 끝나면
            for (int a = 0; a < line.size(); a++) {
              ...
              for (string word : words) {
                ...
                if ((find(stopwords_list.begin(), stopwords_list.end(), word) ==
                     stopwords_list.end()) &&
                    (find(special.begin(), special.end(), word) ==
                     special.end())) {
                  ++word_in_ham[word];
                }
              }
          ...
          if ((line.size() > 1) &&
              (line.at(line.size() - 2)[line.at(line.size() - 2).size() - 1] ==
               '\"') &&
              (line.at(line.size() - 1) == t1)) { // text가 끝나면
  ...
  vector<pair<string, int>> words_in_ham_order(word_in_ham.begin(),
                                               word_in_ham.end());
  vector<pair<string, int>> words_in_spam_order(word_in_spam.begin(),
                                                word_in_spam.end());
  sort(words_in_ham_order.begin(), words_in_ham_order.end(), cmp);
  cout << words_in_ham_order.at(0).first << " : "
       << words_in_ham_order.at(0).second << endl;
